# generate_images.py - Version finale corrigée
import os
import joblib
import pandas as pd
import numpy as np
...
# ⭐ CORRECTION CRITIQUE: Conversion de la cible en valeurs binaires (0/1)
y = (y == 'yes').astype(int)
print("Distribution de la cible après conversion:", y.value_counts())
...
def reproduce_preprocessing(X):
    ...
    # 4. Nettoyage final - suppression de colonnes indésirables si elles existent
    columns_to_remove = ['y']  # La cible ne doit pas être dans les features
...
# 7. Génération de la courbe ROC (version corrigée)
def generate_roc():
    try:
        ...
        # ⭐ CORRECTION: Spécification explicite de pos_label pour éviter l'ambiguïté
        fpr, tpr, _ = roc_curve(y, probas, pos_label=1)
        roc_auc = auc(fpr, tpr)
...
# 8. Génération du plot SHAP (version robuste)
def generate_shap():
    try:
